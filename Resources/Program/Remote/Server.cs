...
namespace ManageMaterialPBA.Resources.Program.Remote
{
    public partial class Server : Form
    {
        ...
        string _IP, _Port, _Pass;

        public Server(string IP, string Port, string Pass)
        {
            InitializeComponent();
            _IP = IP;
            _Port = Port;
        ...
        private void Server_Load(object sender, EventArgs e)
        {
            ...
            this.TxtIPAddress.Text = _IP;
            this.txtPassword.Text = _Pass;
        ...
        {//Connect to the server, send our password and get ready to rock and roll
            try
            {
                ...
                if (Helper.IsIP4Address(_IP))
                    this.Tcpclient = new TcpClient(_IP, int.Parse(_Port));
                else
                {
                    string IP = Helper.GetIP(_IP);
                    if (IP.Length == 0)
                    {
                        MessageBox.Show("Sorry we cannot resolve the name '" + _IP + "' to an IP-Address", "Remote desktop", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    this.Tcpclient = new TcpClient(_IP, int.Parse(_Port));
                }
                ...
                this.LblTitle.Text = _IP + ":" + _Port;
                this.Text = _IP + ":" + _Port;
                SW.Write(Helper.XorString("CMD PASSWORD " + _Pass, 45, true) + "\n");
                SW.Flush();
                Settings.IP = _IP;
                Settings.Port = int.Parse(_Port);
                Settings.PassWord = _Pass;
        ...
        private void theImage_MouseDown_1(object sender, MouseEventArgs e)
        {
            if (Settings.Connected)
                this.Invoke((MethodInvoker)delegate() { GroupConected.Visible = false; GroupConected.Enabled = false; });
            if (!Settings.SendKeysAndMouse)
        ...
        private void theImage_MouseUp_1(object sender, MouseEventArgs e)
        {
        ...
        private void theImage_MouseMove_1(object sender, MouseEventArgs e)
        {
            if (!Settings.SendKeysAndMouse)
                return;
            if (this.WindowState == FormWindowState.Maximized)
